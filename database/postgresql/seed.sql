...
ON CONFLICT (branch_id) DO UPDATE SET branch_name = EXCLUDED.branch_name, email = EXCLUDED.email;
...
ON CONFLICT (customer_id) DO UPDATE SET first_name = EXCLUDED.first_name, last_name = EXCLUDED.last_name, email = EXCLUDED.email;
...
ON CONFLICT (user_id) DO UPDATE SET full_name = EXCLUDED.full_name, username = EXCLUDED.username, email = EXCLUDED.email, password_hash = EXCLUDED.password_hash, role = EXCLUDED.role, account_status = EXCLUDED.account_status;
...
ON CONFLICT (rate_id) DO UPDATE SET tier_name = EXCLUDED.tier_name, cost_per_unit = EXCLUDED.cost_per_unit;
...
ON CONFLICT (meter_id) DO UPDATE SET meter_number = EXCLUDED.meter_number, last_reading = EXCLUDED.last_reading;
...
ON CONFLICT (usage_id) DO UPDATE SET units_used = EXCLUDED.units_used, current_reading = EXCLUDED.current_reading;
...
ON CONFLICT (bill_id) DO UPDATE SET total_amount = EXCLUDED.total_amount, amount_paid = EXCLUDED.amount_paid, balance_due = EXCLUDED.balance_due, payment_status = EXCLUDED.payment_status;
...
ON CONFLICT (payment_id) DO UPDATE SET amount_paid = EXCLUDED.amount_paid, payment_method = EXCLUDED.payment_method;
...
ON CONFLICT (report_id) DO UPDATE SET report_status = EXCLUDED.report_status, severity = EXCLUDED.severity;
...
VALUES
('NOT-1001', 'BILL-1001', 'CUS-1001', 'SMS', 'Your water bill is due on 2026-04-20.', 'SENT', CURRENT_TIMESTAMP),
('NOT-1002', 'BILL-1002', 'CUS-1002', 'EMAIL', 'Your water bill remains unpaid. Please settle your balance.', 'PENDING', NULL)
ON CONFLICT (notification_id) DO UPDATE SET notification_message = EXCLUDED.notification_message, sent_status = EXCLUDED.sent_status;
